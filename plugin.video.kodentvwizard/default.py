...
#######################################################################
import xbmc, xbmcaddon, xbmcgui, xbmcplugin, os, base64, sys, xbmcvfs, glob
import shutil
...
from resources.libs import web
...
AUTOUPDATE       = 'No' #uservar.AUTOUPDATE
...
BASEURL2         = base64.b64decode('aHR0cHM6Ly93d3cudWJkZW4uY29t')
INSTALLMETHODS   = ['Herzaman Sor', 'Profili Yenile', 'Zorla Kapat']
...
KODI_VERSION     = wiz.GET_KODI_VERSION_DETAILS()
...
def index():
	...
	addFile("[COLOR white]KODI VERSIYON [/COLOR] [COLOR aqua]" + KODI_VERSION + "[/COLOR]", '', themeit=THEME2)
	addFile("[COLOR white]KODEN TV SURUM :[/COLOR] [COLOR aqua] 5.2020 [/COLOR]", '', themeit=THEME2)
	addFile(wiz.sep(), '', themeit=THEME3)
	addDir ('[COLOR white]YUKLE/GUNCELLE[/COLOR] [COLOR aqua]KODEN TV[/COLOR]', 'builds', icon=ICONBUILDS, themeit=THEME2)
	#addDir ('Maintenance'   ,'maint',    icon=ICONMAINT,    themeit=THEME1)
	addDir ('[COLOR ghostwhite]ILETISIM: [COLOR aqua]KODI KODEN TV[/COLOR][/COLOR]', 'contact', icon=ICON, themeit=THEME2)
	...
	#addFile(wiz.sep(), '', themeit=THEME3)
	addFile('KODEN TV AYARLARI'      ,'settings', icon=ICONSETTINGS, themeit=THEME2)
...
def buildMenu():
	...
	if not WORKINGURL == True:
		addFile("[COLOR white]POWERED BY [/COLOR] [COLOR aqua]- UBDEN[/COLOR]", '', themeit=THEME2)
		#addFile('%s Version: %s' % (MCNAME, KODIV), '', icon=ICONBUILDS, themeit=THEME3)
		#addFile(wiz.sep(), '', themeit=THEME3)
		addFile('URL adresi gecersiz', '', icon=ICONBUILDS, themeit=THEME3)
		...
		#addFile(wiz.sep(), '', themeit=THEME2)
		if third == True:
			for item in addin:
					...
					if SHOW18 == 'true':
						for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
							...
							if kodiv == 18:
								menu = createMenu('install', '', name)
								#addDir('[%s] %s (v%s)' % (float(kodi), name, version), 'viewbuild', name, description=description, fanart=fanart,icon=icon, menu=menu, themeit=THEME2)
								addDir('%s %s' % (name, version), 'viewbuild', name, description=description, fanart=fanart,icon=icon, menu=menu,  themeit=THEME2)
				if count17 > 0:
					#######################
					# Krypton 17 Build List
					#######################
...
def viewBuild(name):
	WORKINGURL = wiz.workingURL(BUILDFILE)
	if not WORKINGURL == True:
		addFile('URL adresi gecersiz', '', themeit=THEME3)
	...
	for version, url, gui, kodi, themefile, icon, fanart, preview, adult, description in match:
		...
		build       = '%s %s' % (name, version)
		if BUILDNAME == name and version > BUILDVERSION:
			#build = '%s [COLOR red][SELECTED v%s][/COLOR]' % (build, BUILDVERSION)
			build = '%s [COLOR red][SELECTED %s][/COLOR]' % (build, BUILDVERSION)
		addFile(name, 'buildinfo', name, description=description, fanart=fanart, icon=icon, themeit=THEME6)
		#addFile(wiz.sep(), '', themeit=THEME3)
		#addDir ('Save Data Menu',       'savedata', icon=ICONSAVE,     themeit=THEME6)
		#addFile('Build Information',    'buildinfo', name, description=description, fanart=fanart, icon=icon, themeit=THEME6)
		...
		addFile(wiz.sep('KURULUM'), '', fanart=fanart, icon=icon, themeit=THEME3)
		addFile('[COLOR magenta]FRESH INSTALL[/COLOR] (Hersey Silinir, Temiz Kurulum)' , 'install', name, 'fresh'  , description=description, fanart=fanart, icon=icon, themeit=THEME2)
		...
		if not themefile == 'http://':
			if wiz.workingURL(themefile) == True:
				addFile(wiz.sep('THEMES'), '', fanart=fanart, icon=icon, themeit=THEME2)
				...
				for themename, themeurl, themeicon, themefanart, themeadult, description in match:
					...
					addFile(themename if not themename == BUILDTHEME else "[B]%s (Installed)[/B]" % themename, 'theme', name, themename, description=description, fanart=themefanart, icon=themeicon, themeit=THEME2)
	setView('files', 'viewType')


def communitybuilds():
	WORKINGURL = wiz.workingURL(COMMUNITYFILE)
	if not WORKINGURL == True:
		addFile('%s Version: %s' % (MCNAME, KODIV), '', icon=ICONBUILDS, themeit=THEME3)
		addFile(wiz.sep(), '', themeit=THEME3)
		addFile('URL adresi gecersiz', '', icon=ICONBUILDS, themeit=THEME3)
					...
					if plugin.lower() == 'section':
						...
						try:
							...
							if os.path.exists(add):
								name   = "[COLOR green][Yuklendi][/COLOR] %s" % name
						except:
							pass
						x += 1
				...
				for name, url, repository, repositoryxml, repositoryurl, icon, fanart, adult, description in match:
					if os.path.exists(os.path.join(ADDONS, plugin)):
						...
						if selected == 0:
							...
							if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Seciminizi Yapin remove the addon_data for:" % COLOR2, "[COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR1, plugin), yeslabel="[B][COLOR green]Yes Remove[/COLOR][/B]", nolabel="[B][COLOR red]No Skip[/COLOR][/B]"):
					...
					if not repository.lower() == 'none' and not os.path.exists(repo):
						...
						if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Seciminizi Yapin install the repository for [COLOR %s]%s[/COLOR]:" % (COLOR2, COLOR1, plugin), "[COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR1, repository), yeslabel="[B][COLOR green]Yes Install[/COLOR][/B]", nolabel="[B][COLOR red]No Skip[/COLOR][/B]"): 
...
def installAddon(name, url):
	...
	DP.create(ADDONTITLE,'[COLOR %s][B]Indiriliyor:[/B][/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, name), '', '[COLOR %s]Lutfen Bekleyin[/COLOR]' % COLOR2)
	urlsplit = url.split('/')
	lib=os.path.join(PACKAGES, urlsplit[-1])
	try: os.remove(lib)
	except: pass
	downloader.download(url, lib, DP)
	title = '[COLOR %s][B]Yukleniyor:[/B][/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, name)
	DP.update(0, title,'', '[COLOR %s]Lutfen Bekleyin[/COLOR]' % COLOR2)
		...
		for depends in match:
			if not 'xbmc.python' in depends:
				...
				# if not os.path.exists(dependspath):
					...
					# try: os.remove(lib)
					# except: pass
					# DP.update(0, '[COLOR %s][B]Downloading Dependency:[/B][/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, depends),'', 'Lutfen Bekleyin')
					# downloader.download(depzip, lib, DP)
					# xbmc.sleep(100)
					# title = '[COLOR %s][B]Installing Dependency:[/B][/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, depends)
					# DP.update(0, title,'', 'Lutfen Bekleyin')
					# percent, errors, error = extract.all(lib,ADDONS,DP, title=title)
					# if KODIV >= 17: wiz.addonDatabase(depends, 1)
					# installed(depends)
					# installDep(depends)
...
def writeAdvanced(name, url):
	ADVANCEDWORKING = wiz.workingURL(url)
	if ADVANCEDWORKING == True:
		if os.path.exists(ADVANCED): choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Seciminizi Yapin overwrite your current Advanced Settings with [COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, name), yeslabel="[B][COLOR green]Overwrite[/COLOR][/B]", nolabel="[B][COLOR red]Cancel[/COLOR][/B]")
		else: choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Seciminizi Yapin download and install [COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, name), yeslabel="[B][COLOR green]Install[/COLOR][/B]", nolabel="[B][COLOR red]Cancel[/COLOR][/B]")
...
def saveMenu():
	...
	addDir ('Sakla Trakt Data',               'trakt',                icon=ICONTRAKT, themeit=THEME6)
	addDir ('Sakla Real Debrid',              'realdebrid',           icon=ICONREAL,  themeit=THEME6)
	addDir ('Sakla Login Info',               'login',                icon=ICONLOGIN, themeit=THEME6)
	addFile('Import Save Data',              'managedata', 'import', icon=ICONSAVE,  themeit=THEME6)
	addFile('Export Save Data',              'managedata', 'export', icon=ICONSAVE,  themeit=THEME6)
	addFile('- Click to toggle settings -', '', themeit=THEME3)
	addFile('Save Trakt: %s' % trakt.replace('true',on).replace('false',off)                       ,'togglesetting', 'keeptrakt',      icon=ICONTRAKT, themeit=THEME6)
	addFile('Save Real Debrid: %s' % real.replace('true',on).replace('false',off)                  ,'togglesetting', 'keepdebrid',     icon=ICONREAL,  themeit=THEME6)
	addFile('Save Login Info: %s' % login.replace('true',on).replace('false',off)                  ,'togglesetting', 'keeplogin',      icon=ICONLOGIN, themeit=THEME6)
	addFile('Sakla \'Sources.xml\': %s' % sources.replace('true',on).replace('false',off)           ,'togglesetting', 'keepsources',    icon=ICONSAVE,  themeit=THEME6)
	addFile('Sakla \'Profiles.xml\': %s' % profiles.replace('true',on).replace('false',off)         ,'togglesetting', 'keepprofiles',   icon=ICONSAVE,  themeit=THEME6)
	addFile('Sakla \'Advancedsettings.xml\': %s' % advanced.replace('true',on).replace('false',off) ,'togglesetting', 'keepadvanced',   icon=ICONSAVE,  themeit=THEME6)
	addFile('Sakla \'Favourites.xml\': %s' % favourites.replace('true',on).replace('false',off)     ,'togglesetting', 'keepfavourites', icon=ICONSAVE,  themeit=THEME6)
	#addFile('Sakla Super Favourites: %s' % super.replace('true',on).replace('false',off)            ,'togglesetting', 'keepsuper',      icon=ICONSAVE,  themeit=THEME6)
	addFile('Sakla Installed Repo\'s: %s' % repos.replace('true',on).replace('false',off)           ,'togglesetting', 'keeprepos',      icon=ICONSAVE,  themeit=THEME6)
	addFile('Sakla My \'WhiteList\': %s' % whitelist.replace('true',on).replace('false',off)        ,'togglesetting', 'keepwhitelist',  icon=ICONSAVE,  themeit=THEME6)
	if whitelist == 'true':
		addFile('Edit My Whitelist',        'whitelist', 'edit',   icon=ICONSAVE,  themeit=THEME6)
		addFile('View My Whitelist',        'whitelist', 'view',   icon=ICONSAVE,  themeit=THEME6)
		addFile('Clear My Whitelist',       'whitelist', 'clear',  icon=ICONSAVE,  themeit=THEME6)
		addFile('Import My Whitelist',      'whitelist', 'import', icon=ICONSAVE,  themeit=THEME6)
		addFile('Export My Whitelist',      'whitelist', 'export', icon=ICONSAVE,  themeit=THEME6)
	setView('files', 'viewType')
...
def removeAddonMenu():
	...
	if len(selected) > 0:
		...
		else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Seciminizi Yapin[COLOR %s]Zorla Kapat[/COLOR] yada [COLOR %s]Profili Yenile[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR green]Profili Yenile[/COLOR][/B]", nolabel="[B][COLOR red]Zorla Kapat[/COLOR][/B]")
...
def buildWizard(name, type, theme=None, over=False):
	...
	if type == 'gui':
		if name == BUILDNAME:
			if over == True: yes = 1
			else: yes = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Seciminizi Yapin apply the guifix for:' % COLOR2, '[COLOR %s]%s[/COLOR]?[/COLOR]' % (COLOR1, name), nolabel='[B][COLOR red]No, Cancel[/COLOR][/B]',yeslabel='[B][COLOR green]Apply Fix[/COLOR][/B]')
		else: 
		...
		if yes:
			...
			if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
			DP.create(ADDONTITLE,'[COLOR %s][B]Downloading GuiFix:[/B][/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, name),'', 'Lutfen Bekleyin')
			...
			else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]The Gui fix has been installed.  Would you like to Reload the profile or Force Close Kodi?[/COLOR]" % COLOR2, yeslabel="[B][COLOR red]Profili Yenile[/COLOR][/B]", nolabel="[B][COLOR green]Zorla Kapat[/COLOR][/B]")
		...
		if warning == True:
			...
			#else: yes_pressed = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Seciminizi Yapin Download and Install:' % COLOR2, '[COLOR %s]%s v%s[/COLOR]?[/COLOR]' % (COLOR1, name, wiz.checkBuild(name,'version')), nolabel='[B][COLOR red]No, Cancel[/COLOR][/B]',yeslabel='[B][COLOR green]Yes, Install[/COLOR][/B]')
			else: yes_pressed = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Seciminizi Yapin Download and Install:' % COLOR2, '[COLOR %s]%s %s[/COLOR]?[/COLOR]' % (COLOR1, name, wiz.checkBuild(name,'version')), nolabel='[B][COLOR red]No, Cancel[/COLOR][/B]',yeslabel='[B][COLOR green]Yes, Install[/COLOR][/B]')
		if yes_pressed:
			wiz.clearS('build')
			...
			if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
			#DP.create(ADDONTITLE,'[COLOR %s][B]Indiriliyor:[/B][/COLOR] [COLOR %s]%s v%s[/COLOR]' % (COLOR2, COLOR1, name, wiz.checkBuild(name,'version')),'', 'Lutfen Bekleyin')
			DP.create(ADDONTITLE,'[COLOR %s][B]Indiriliyor:[/B][/COLOR] [COLOR %s]%s %s[/COLOR]' % (COLOR2, COLOR1, name, wiz.checkBuild(name,'version')),'', 'Lutfen Bekleyin')
			lib=os.path.join(PACKAGES, '%s.zip' % zipname)
			try: os.remove(lib)
			except: pass
			downloader.download(buildzip, lib, DP)
			xbmc.sleep(500)
			#title = '[COLOR %s][B]Yukleniyor:[/B][/COLOR] [COLOR %s]%s v%s[/COLOR]' % (COLOR2, COLOR1, name, wiz.checkBuild(name,'version'))
			title = '[COLOR %s][B]Yukleniyor:[/B][/COLOR] [COLOR %s]%s %s[/COLOR]' % (COLOR2, COLOR1, name, wiz.checkBuild(name,'version'))
			DP.update(0, title,'', 'Lutfen Bekleyin')
			...
			if int(float(percent)) > 0:
				...
				else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Seciminizi Yapin [COLOR %s]Zorla Kapat[/COLOR] yada [COLOR %s]Profili Yenile[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR red]Profili Yenile[/COLOR][/B]", nolabel="[B][COLOR green]Zorla Kapat[/COLOR][/B]")
				if todo == 1: wiz.reloadFix()
		...
		else: installtheme = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Seciminizi Yapin install the theme:' % COLOR2, '[COLOR %s]%s[/COLOR]' % (COLOR1, theme), 'for [COLOR %s]%s v%s[/COLOR]?[/COLOR]' % (COLOR1, name, wiz.checkBuild(name,'version')), yeslabel="[B][COLOR green]Install Theme[/COLOR][/B]", nolabel="[B][COLOR red]Cancel Themes[/COLOR][/B]")
		if installtheme:
			...
			if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
			DP.create(ADDONTITLE,'[COLOR %s][B]Indiriliyor:[/B][/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, theme),'', 'Lutfen Bekleyin')
			lib=os.path.join(PACKAGES, '%s.zip' % zipname)
			try: os.remove(lib)
			except: pass
...
def thirdPartyInstall(name, url):
	...
	type = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Seciminizi Yapin preform a [COLOR %s]Fresh Install[/COLOR] or [COLOR %s]Normal Install[/COLOR] for:[/COLOR]" % (COLOR2, COLOR1, COLOR1), "[COLOR %s]%s[/COLOR]" % (COLOR1, name), yeslabel="[B][COLOR green]Fresh Install[/COLOR][/B]", nolabel="[B][COLOR red]Normal Install[/COLOR][/B]")
	if type == 1:
		freshStart('third', True)
	wiz.clearS('build')
	zipname = name.replace('\\', '').replace('/', '').replace(':', '').replace('*', '').replace('?', '').replace('"', '').replace('<', '').replace('>', '').replace('|', '')
	if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
	DP.create(ADDONTITLE,'[COLOR %s][B]Indiriliyor:[/B][/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, name),'', 'Lutfen Bekleyin')
	lib=os.path.join(PACKAGES, '%s.zip' % zipname)
	try: os.remove(lib)
	except: pass
	downloader.download(url, lib, DP)
	...
	if int(float(percent)) > 0:
		...
		if int(float(errors)) > 0:
			yes=DIALOG.yesno(ADDONTITLE, '[COLOR %s][COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, name), 'Tamamlanan: [COLOR %s]%s%s[/COLOR] [Hata:[COLOR %s]%s[/COLOR]]' % (COLOR1, percent, '%', COLOR1, errors), 'Would you like to view the errors?[/COLOR]', nolabel='[B][COLOR red]No Thanks[/COLOR][/B]',yeslabel='[B][COLOR green]View Errors[/COLOR][/B]')
			if yes:
				if isinstance(errors, unicode):
					error = error.encode('utf-8')
				wiz.TextBox(ADDONTITLE, error)
...
def apkInstaller(apk, url):
	...
	if wiz.platform() == 'android':
		yes = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Seciminizi Yapin download and install:" % COLOR2, "[COLOR %s]%s[/COLOR]" % (COLOR1, apk), yeslabel="[B][COLOR green]Download[/COLOR][/B]", nolabel="[B][COLOR red]Cancel[/COLOR][/B]")
		...
		DP.create(ADDONTITLE,'[COLOR %s][B]Indiriliyor:[/B][/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, display),'', 'Lutfen Bekleyin')
...
def removeAddonData(addon, name=None, over=False):
	if addon == 'all':
		if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Seciminizi Yapin remove [COLOR %s]ALL[/COLOR] addon data stored in you Userdata folder?[/COLOR]' % (COLOR2, COLOR1), yeslabel='[B][COLOR green]Remove Data[/COLOR][/B]', nolabel='[B][COLOR red]Don\'t Remove[/COLOR][/B]'):
		...
		if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Seciminizi Yapin remove [COLOR %s]ALL[/COLOR] addon data stored in you Userdata folder for uninstalled addons?[/COLOR]' % (COLOR2, COLOR1), yeslabel='[B][COLOR green]Remove Data[/COLOR][/B]', nolabel='[B][COLOR red]Don\'t Remove[/COLOR][/B]'):
			total = 0
		...
		if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Seciminizi Yapin remove [COLOR %s]ALL[/COLOR] empty addon data folders in you Userdata folder?[/COLOR]' % (COLOR2, COLOR1), yeslabel='[B][COLOR green]Remove Data[/COLOR][/B]', nolabel='[B][COLOR red]Don\'t Remove[/COLOR][/B]'):
...
def DONATIONS_LINK1():
	...
	#Common.TextBox('[COLOR aqua][B]Koden TV Smart[/B][/COLOR]',f.text)
	web.function27()
...
def freshStart(install=None, over=False):
	...
	elif install == 'restore': yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Kodi Sifirlanacak" % COLOR2, "Ayarlariniz Silinecektir", "Emin Misiniz ?[/COLOR]", nolabel='[B][COLOR red]Hayir, Iptal[/COLOR][/B]', yeslabel='[B][COLOR green]Islemi Baslat[/COLOR][/B]')
	elif install: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Kodi Sifirlanacak" % COLOR2, "Ayarlariniz Silinecektir", "[COLOR %s]%s[/COLOR]? Yukleniyor Emin Misiniz?" % (COLOR1, install), nolabel='[B][COLOR red]Hayir, Iptal[/COLOR][/B]', yeslabel='[B][COLOR green]Islemi Baslat[/COLOR][/B]')
	...
	if yes_pressed:
		...
		DP.create(ADDONTITLE,"[COLOR %s]Calculating files and folders" % COLOR2,'', 'Lutfen Bekleyin![/COLOR]')
		...
		for root, dirs, files in os.walk(xbmcPath,topdown=True):
			...
			for name in files:
				...
				x = len(fold)-1
				if name == 'sources.xml' and fold[-1] == 'userdata' and KEEPSOURCES == 'true': wiz.log("Sakla Sources: %s" % os.path.join(root, name), xbmc.LOGNOTICE)
				elif name == 'favourites.xml' and fold[-1] == 'userdata' and KEEPFAVS == 'true': wiz.log("Sakla Favourites: %s" % os.path.join(root, name), xbmc.LOGNOTICE)
				elif name == 'profiles.xml' and fold[-1] == 'userdata' and KEEPPROFILES == 'true': wiz.log("Sakla Profiles: %s" % os.path.join(root, name), xbmc.LOGNOTICE)
				elif name == 'advancedsettings.xml' and fold[-1] == 'userdata' and KEEPADVANCED == 'true':  wiz.log("Sakla Advanced Settings: %s" % os.path.join(root, name), xbmc.LOGNOTICE)
				elif name in LOGFILES: wiz.log("Sakla Log File: %s" % name, xbmc.LOGNOTICE)
					...
					DP.update(int(wiz.percentage(del_file, total_files)), '', '[COLOR %s]Dosya: [/COLOR][COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, name), '')
			...
			if DP.iscanceled(): 
				DP.close()
				wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Temiz Kurulum Iptal Edildi[/COLOR]" % COLOR2)
				return False
		for root, dirs, files in os.walk(xbmcPath,topdown=True):
			dirs[:] = [d for d in dirs if d not in EXCLUDES]
			for name in dirs:
...
####THANKS GUYS @ NaN #######
def clearCache():
	if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Seciminizi Yapin clear cache?[/COLOR]' % COLOR2, nolabel='[B][COLOR red]No, Cancel[/COLOR][/B]', yeslabel='[B][COLOR green]Clear Cache[/COLOR][/B]'):
		wiz.clearCache()
		#clearThumb()

def totalClean():
	if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Seciminizi Yapin clear cache, packages and thumbnails?[/COLOR]' % COLOR2, nolabel='[B][COLOR red]Cancel Process[/COLOR][/B]',yeslabel='[B][COLOR green]Clean All[/COLOR][/B]'):
		wiz.clearCache()
...
def clearThumb(type=None):
	...
	else: choice = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Seciminizi Yapin delete the %s and Thumbnails folder?' % (COLOR2, latest), "They will repopulate on the next startup[/COLOR]", nolabel='[B][COLOR red]Don\'t Delete[/COLOR][/B]', yeslabel='[B][COLOR green]Delete Thumbs[/COLOR][/B]')
	if choice == 1:
...
elif mode=='donate1'        : DONATIONS_LINK1()
elif mode=='donate2'        : DONATIONS_LINK2()
elif mode=='donate3'        : DONATIONS_LINK3()
