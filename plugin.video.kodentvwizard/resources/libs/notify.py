...
def firstRun():
	class MyWindow(xbmcgui.WindowXMLDialog):
		def __init__(self, *args, **kwargs):
			...
			self.msg   = "%s:\n\nHosgeldiniz 'Kurulumu Baslat' butonunu tiklayarak hemen baslayabilir yada 'Daha Sonra' secenigiyle eklentiyi kapatabilirsiniz.\n\nBizi Tercih Ettiginiz Icin Tesekkurler %s." % (ADDONTITLE, ADDONTITLE)
		...
		def showdialog(self):
			...
			self.getControl(self.image).setColorDiffuse('FFFFFFFF')
			self.getControl(self.textbox).setText(self.msg)
			self.getControl(self.titlebox).setLabel(self.title)
