...
def allWithProgress(_in, _out, dp, ignore, title):
	...
	for item in zin.infolist():
		...
		skip = False
		line1  = '%s [COLOR %s][B][Hata:%s][/B][/COLOR]' % (title, COLOR2, errors)
		line2  = '[COLOR %s][B]Dosya:[/B][/COLOR] [COLOR %s]%s/%s[/COLOR] ' % (COLOR2, COLOR1, count, int(nFiles))
		line2 += '[COLOR %s][B]Boyut:[/B][/COLOR] [COLOR %s]%s/%s[/COLOR]' % (COLOR2, COLOR1, wiz.convertSize(size), zipsize)
		...
		if skip == True: wiz.log("Skipping: %s" % item.filename, xbmc.LOGNOTICE)
			...
			try:
				...
				wiz.log('Ayiklama Hatasi: %s(%s)' % (item.filename, str(e)), xbmc.LOGERROR)
				pass
		dp.update(prog, line1, line2, line3)
		if dp.iscanceled(): break
	if dp.iscanceled(): 
		dp.close()
		wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Ayiklama Iptal[/COLOR]" % COLOR2)
		sys.exit()
