...
AUTOINSTALL    = 'No' #uservar.AUTOINSTALL
...
def reloadFix(default=None):
	DIALOG.ok(ADDONTITLE, "[COLOR %s]UYARI: Bazen profili yeniden yuklemek Kodinin cokmesine neden olur. Lutfen profili yukleniyor herhangi bir butona basmayin![/COLOR]" % COLOR2)
	if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
	if default == None:
		lookandFeelData('save')
	redoThumbs()
