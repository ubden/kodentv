...
class podcast:
    ...
    def pb_root(self):
        try:
            ...
            for i in cats:
                self.list.append({'name': i[0], 'url': self.pbcats_link % i[1], 'image': 'DefaultVideoPlaylists.png', 'action': 'podbay&podcastlist=%s' % i[1]})
    ...
    def pcocats_list(self):
        ...
        for i in cats:
            self.list.append({'name': i[0], 'url': self.pcocats_link % i[1], 'image': 'DefaultVideoPlaylists.png', 'action': 'podcastOne&podcastlist=%s' % i[1]})
        self.addDirectory(self.list)
        return self.list
