...
def SKindex():
    addon_log("SKindex")
    addDir('Favorites','Favorites',4,'http://i.imgur.com/C4hLLSK.png',  FANART,'','','','')
...
def addDir(name,url,mode,iconimage,fanart,description,genre,date,credits,showcontext=False):
        ...
        if showcontext:
            ...
            if not name in FAV:
                contextMenu.append(('Add to RetroMania Favourites','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie=""):
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                ...
                contextMenu.append(('Add to RetroMania Favourites','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
