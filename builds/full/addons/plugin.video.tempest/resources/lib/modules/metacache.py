...
def fetch(items, lang='en', user=''):
    ...
    for i in range(0, len(items)):
        try:
            dbcur.execute(
                "SELECT * FROM meta WHERE (imdb = '%s' and lang = '%s' and user = '%s' "
                "and not imdb = '0') or (tvdb = '%s' and lang = '%s' and user = '%s' and not tvdb = '0')" %
...
def insert(meta):
    try:
        ...
        dbcur = dbcon.cursor()
        dbcur.execute(
            "CREATE TABLE IF NOT EXISTS meta (""imdb TEXT, ""tvdb TEXT, ""lang TEXT, """
            "user TEXT, ""item TEXT, ""time TEXT, ""UNIQUE(imdb, tvdb, lang, user)"");")
        t = int(time.time())
        for m in meta:
            try:
                if "user" not in m:
                ...
                try:
                    dbcur.execute(
                        "DELETE * FROM meta WHERE (imdb = '%s' and lang = '%s' and user = '%s' and not imdb = '0') "
                        "or (tvdb = '%s' and lang = '%s' and user = '%s' and not tvdb = '0')" %
